...
% Inverse FFT
dataInTimeDomain = ifft(fourPilotInsertedData);
...
for iter=1:iterNum
    for i = 1:length(SNR)
        ...
        for t=1: (m/subcarrierNumber)
            pilot_new(t,:) = ( fft_wcycpre(t,1) + fft_wcycpre(t,pilot1) + fft_wcycpre(t,pilot2) + fft_wcycpre(t,pilot3) ) / pilotNumber;
        end
...
title('4-Pilot & 5-Tap with Cyclic Prefix Insertion');
